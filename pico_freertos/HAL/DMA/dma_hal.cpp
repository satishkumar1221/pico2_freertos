 ...
 * Date:        6/10/2025
 * Description: Contains logic for dimrect memory access in hardware abstraction
 *layer
 *
 * Revision History:
...
Job_status_LLD V_job_complete_status_dma_spi = Job_status_LLD_FAILED;
/**
 ...
 * each containing configuration settings for a specific peripheral. It iterates
 through
 * the provided configurations, configuring the DMA channel and
 enabling/disabling interrupts
 * according to the specified settings.
 *
 * @param ptr_dma_structs Pointer to an array of `@ref st_DMA_config`
 structures.
 ...
 * digraph G {
   ...
   "Check if all configurations processed" -> "Process next configuration"
 [label="No"]; "Check if all configurations processed" -> "Return success code";
...
void DMA_config ::Configure_DMA_based_on_peripherals(
    const st_DMA_config *ptr_dma_structs) {
  uint32_t bitmask;
  ...
  case CONFIGURE_SPI_BUS:
    if (ptr_dma_structs->peri_bus_configured == SPI_BUS_TO_BE_CONFIGURED) {
      ...
      dma_channel_config c =
          dma_channel_get_default_config(ptr_dma_structs->channel_used_tx);
      channel_config_set_transfer_data_size(&c, DMA_SIZE_8);

      dma_channel_configure(ptr_dma_structs->channel_used_tx, &c,
      ...
      /*Configure DMA_RX */
      auto rx =
          dma_channel_get_default_config(ptr_dma_structs->channel_used_rx);
      dma_channel_configure(ptr_dma_structs->channel_used_rx, &rx,
                            &spi_get_hw(spi_default)->dr,    // write address
 ...
 *
 * This method sets up the DMA controller and initializes its registers
 ...
 * @brief Initializes the DMA module.
 *
 * @details This method initializes the DMA controller and sets up its registers
 * according to the specified configuration.
 */
...
void DMA_config ::DMA_Init() {
  ...
  for (itr = 0; itr < DMA_CHANNELS; itr++) {
    lib_memset(C_DMA_CONFIG[itr].ptr_txbuffer, blank_space,
               C_DMA_CONFIG[itr].size_txbuffer);
    lib_memset(C_DMA_CONFIG[itr].ptr_destbuffer, blank_space,
               C_DMA_CONFIG[itr].size_rxbuffer);
  }
  ...
  if (C_DMA_CONFIG[itr].peripherals_used >= 0u) {
    if (is_channel_free(C_DMA_CONFIG[SPI_CHANNEL].channel_used_tx) &&
        (is_channel_free(C_DMA_CONFIG[SPI_CHANNEL].channel_used_rx))) {
...
void DMA_config ::start_dma() {
  ...
  uint32_t tx_mask;
  uint32_t rx_mask;
  V_job_complete_status_dma_spi = Job_status_LLD_BUSY;
  for (itr = 0; itr < TOTAL_DMA_CHANNELS; itr++) {
    ...
    dma_channel_wait_for_finish_blocking(rx_mask);
    /* As Blocking call is used make the flag to true  */

    if (dma_channel_is_busy(tx_mask)) {
      panic("RX completed before TX");
    }
        V_job_complete_status_dma_spi = Job_status_LLD_OK; /*DMA JOB COMPLETE */
  }
}

void DMA_config ::abort_dma(uint8_t channel) { dma_channel_abort(channel); }
...
bool DMA_config ::is_channel_busy(uint8_t channel) { return (false); }
...
bool DMA_config ::get_channel_irq_status(uint8_t channel) { return (false); }

uint32_t DMA_config ::dma_get_interrupt_number(uint32_t irq_index) {
  return (false);
}

void DMA_config ::sniffer_enable_dma(uint32_t channel, uint32_t mode,
