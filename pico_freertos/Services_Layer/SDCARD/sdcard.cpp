...
uint8_t SD_Card_Interface ::  Validate_sector_tobewritten(header_Struct &header_Struct )
{
    auto itr =0 ; 
    MemLib obj_mem; 
    const uint64_t total_logical_sectors = ((CONVERT_GB_TO_BYTES - BOOK_KEEPING_BLOCKS)/ (SECTOR_SIZE_PHYSICAL)); 
    ...
    for(itr = 0 ; itr <TOTAL_NUMBER_OF_BLOCKS_CONFIGURED ; itr++)
    {
         if(NV_cache_struct[itr].header.block_id == header_Struct.block_id)
         {
            ...
            if(NV_cache_struct[itr].cache_value == CACHE_INVALIDATED_BLOCK_NOT_PROGRAMMED)
            {
                flag_found = false ; 
            }
            else 
         ...
         if(false == flag_found)
         {
          ...
          NV_cache_struct[header_Struct.block_id].header.address_first_block = offset; 
          ...
          {
            obj_mem.memcopy_overlapprotection(((&V_TX_BUFFER_DMA_LAYER[0]) ),&NV_cache_struct[0],(sizeof(NV_cache_struct)));
            CRC16 obj_crc_16;
            V_sttag_sdcard_packet.CRC = obj_crc_16.processBuffer(reinterpret_cast<uint8_t*>(&NV_cache_struct[0]) , sizeof(NV_cache_struct)); 
          }
...
const uint32_t payload_len = memif_job->size;
    const uint32_t overhead = sizeof(header_Struct) + sizeof(V_sttag_sdcard_packet.end_pattern) /*end*/ + sizeof(V_sttag_sdcard_packet.CRC) /*crc*/;
   ...
   // NV_cache_struct[V_sttag_sdcard_packet.header.block_id].header =
    ...
    NV_cache_struct[V_sttag_sdcard_packet_multiple_Sectorwrites.header.block_id]
        .header = V_sttag_sdcard_packet_multiple_Sectorwrites.header;
          /*Write code for multi blocks  */
         auto result = Validate_sector_tobewritten(V_sttag_sdcard_packet_multiple_Sectorwrites.header);
  }
}
...
void SD_Card_Interface ::Pack_UserData_CRC(sttag_Memif *memif_job) {
  ...
  if (memif_job->size <= (SECTOR_SIZE_PHYSICAL - overhead)) {
    ...
    memcopy_overlapprotection(
        ...
        &V_sttag_sdcard_packet.end_pattern,(sizeof(V_sttag_sdcard_packet.end_pattern)));

         V_sttag_sdcard_packet.CRC = obj_crc_16.processBuffer(
        ((&V_TX_BUFFER_DMA_LAYER[0]) + sizeof(header_Struct)), memif_job->size +sizeof(V_sttag_sdcard_packet.end_pattern));
