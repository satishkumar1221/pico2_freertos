 ...
 *
 * @param[in] pvParameters Pointer to task parameters (currently unused, should
 * be NULL).
 * @return None
 */
...
void OS_20ms_task_c0(void *pvParameters) {
  ...
  for (;;) {
    ...
    NVM_MainFunction();
    /*MemIf should be called later on. MemIf should never be called befor NVM as
     * the data validation is done as a part of NVM */
    MemIf_MainFunction();
    /*sd card main function to process the request related to sd card . As
     * filesytem is not used user must specify the persistant ID of the blocks
     * or can be auto generated*/
